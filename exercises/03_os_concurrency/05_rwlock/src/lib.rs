...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if self.state.compare_exchange(state, state + 1, Ordering::AcqRel, Ordering::Acquire).is_ok() {
                break RwLockReadGuard { lock: self };
            }
        }
    }
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            ...
            // Try to transition from WRITER_WAITING to WRITER_HOLDING (no readers or other writers)
            // or from 0 to WRITER_HOLDING if another writer cleared the waiting flag
            if self.state.compare_exchange(state, (state & !WRITER_WAITING) | WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire).is_ok() {
                break RwLockWriteGuard { lock: self };
            }
        }
    }
}
