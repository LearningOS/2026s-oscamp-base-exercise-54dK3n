...
impl Sv39PageTable {
    ...
    /// 从 39 位虚拟地址中提取页内偏移。
    ///
    /// 提示：右移 PAGE_OFFSET_BITS 位，然后与 (1 << PAGE_OFFSET_BITS) - 1 做掩码。
    pub fn extract_offset(va: u64) -> u32 {
        // TODO
        (va & ((1 << 12) - 1)) as u32
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                ...
                return TranslateResult::Ok((ppn << 12) + (va & offset_mask));
            }
            current_ppn = pte >> PPN_SHIFT;
        }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let level1_ppn = if pte & PTE_V == 0 {
            let next_ppn = self.alloc_node();
            let root = self.nodes.get_mut(&self.root_ppn).unwrap();
            root.entries[vpn2] = (next_ppn << PPN_SHIFT) | PTE_V;
            next_ppn
        } else {
            pte >> PPN_SHIFT
        };
